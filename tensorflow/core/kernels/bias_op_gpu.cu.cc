...
__global__ void BiasGradNCHW_SharedAtomics(const T* output_backprop,
  ...
  if (thread_index < 16) s_data[thread_index] += s_data[thread_index + 16]; __syncwarp(0xFFFF);
  if (thread_index < 8) s_data[thread_index] += s_data[thread_index + 8]; __syncwarp(0xFF);
  if (thread_index < 4) s_data[thread_index] += s_data[thread_index + 4]; __syncwarp(0xF);
  if (thread_index < 2) s_data[thread_index] += s_data[thread_index + 2]; __syncwarp(0x3);
